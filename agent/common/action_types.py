...
engine (ActionType enum) and agent (string-based) layers.
...
available to all agent code that references these sets, without
...
#
# These are generic event_type labels produced by the memory text parser
# (_parse_text_public_entry). They are NOT action_type strings and should
# NOT be confused with AGENT_ACTION_TYPES entries. They coexist alongside
# ActionType-derived sets for event classification in memory.py.
# ---------------------------------------------------------------------------

#: Generic event-type labels for parsed speech events.
#: "speech" is produced by _parse_text_public_entry for unrecognized
#: speech-like text entries that don't map to a specific ActionType.
EVENT_TYPE_SPEECH: str = "speech"

#: Generic event-type label for parsed vote events.
#: "vote" is produced by _parse_text_public_entry for vote-like text
#: entries that don't map to a specific ActionType.
EVENT_TYPE_VOTE: str = "vote"

#: The union of SPEECH_ACTION_TYPES + the generic "speech" event type.
#: Used by memory.py to classify both ActionType-derived and text-parsed
#: speech events.
PUBLIC_SPEECH_EVENT_TYPES: frozenset[str] = SPEECH_ACTION_TYPES | {EVENT_TYPE_SPEECH}

#: The union of VOTE_ACTION_TYPES + the generic "vote" event type.
#: Used by memory.py to classify both ActionType-derived and text-parsed
#: vote events.
