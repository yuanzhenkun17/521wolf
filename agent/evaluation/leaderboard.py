...
from agent.evaluation.confidence_calibration import merge_calibration_reports
from agent.evaluation.statistics import intervals_separate, mean_ci95, wilson_ci95
...
def annotate_vs_baseline(entries: list[LeaderboardEntry], baseline_version: str) -> list[LeaderboardEntry]:
    """Add score delta and conservative significance flags against a baseline."""
    baseline = next((entry for entry in entries if entry.version == baseline_version), None)
    if baseline is None:
        return entries
    base_score = baseline.role_weighted_score or baseline.avg_score
    base_ci = baseline.role_weighted_score_ci95 if baseline.role_weighted_score else baseline.avg_score_ci95
    for entry in entries:
        score = entry.role_weighted_score or entry.avg_score
        ci = entry.role_weighted_score_ci95 if entry.role_weighted_score else entry.avg_score_ci95
        entry.score_delta_vs_base = score - base_score
        entry.significant_vs_base = intervals_separate(ci, base_ci)
    return entries
