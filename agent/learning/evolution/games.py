...
from agent.learning.evolution.consolidation import consolidate_from_mid_memories, write_consolidation
...
async def run_selfplay(
    ...
    *,
    ...
    if config.db_path is not None:
        ...
        run_conn = get_connection(config.db_path)
...
async def _run_single_game(
    *,
    ...
    decision_recorder = AgentDecisionRecorder(conn=conn, game_id=game_id)
    ...
    if conn is not None:
        ...
        gs.insert_players(game_id, player_roles, deaths=deaths)
    ...
    # mid_memory only when enable_mid_memory=True (evolution needs it for consolidation)
