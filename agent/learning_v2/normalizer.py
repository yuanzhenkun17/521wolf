...
def normalize_decisions(bundle: GameEvidenceBundle) -> list[DecisionEvidenceInput]:
    """Build standardized evidence inputs from archive and decision log rows."""
    agent_by_id = {
    ...
    for fallback_index, archived in enumerate(archive_decisions, start=1):
        ...
        merged = {**archived, **{k: v for k, v in agent_row.items() if v is not None}}
        ...
        normalized.append(
            DecisionEvidenceInput(
                ...
                agent_reasoning=AgentReasoning(
                    ...
                    rejected_reasons=[str(x) for x in (merged.get("rejected_reasons") or [])],
