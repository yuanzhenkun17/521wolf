...
Submodules:
    _replay.py  — scenario_replay_node and replay helpers
"""
...
from ._replay import scenario_replay_node  # noqa: F401 — re-export for builder.py
from ._decide import decide_node, _freeze_baseline, _resolve_baseline_skill_dir  # noqa: F401
from ._shared import _registry, _persist_run_state, _baseline_skill_dir  # noqa: F401
...
async def init_evolve_node(state: EvolveState) -> dict:
    ...
    cfg.setdefault("regression_threshold", 0.05)
    cfg.setdefault("convergence_rounds", 3)
    cfg.setdefault("min_improvement_ratio", 0.01)
    ...
    state["config"] = cfg
    state["run_id"] = run_id
    # Propagate evolution history for convergence detection
    if "evolution_history" in cfg and not state.get("evolution_history"):
        state["evolution_history"] = cfg.pop("evolution_history")
...
async def training_node(state: EvolveState) -> dict:
    ...
    try:
        ...
        _log.error("training: failed for role=%s: %s", role, exc, exc_info=True)
        state["training_games"] = [existing_games[seed] for seed in sorted(existing_games)]
        message = f"training: {exc}"
        state.setdefault("errors", []).append(message)
        _record_diagnostic(state, kind="training_error", stage="training.run_games", message=message, exc=exc)
...
async def consolidate_node(state: EvolveState) -> dict:
    ...
    try:
        ...
        _log.error("consolidate: failed for role=%s: %s", role, exc, exc_info=True)
...
async def apply_node(state: EvolveState) -> dict:
    ...
    try:
        ...
        _log.error("apply: failed for role=%s: %s", role, exc, exc_info=True)
        message = f"apply: {exc}"
        state.setdefault("errors", []).append(message)
...
async def _replay_scenario_result(
    ...
    *,
    ...
    Builds decision messages from the frozen snapshot context, runs the LLM
    to produce a candidate decision, and compares it against the original
    baseline decision recorded during training.
    """
...
async def battle_node(state: EvolveState) -> dict:
    ...
    try:
        ...
        _log.error("battle: failed for role=%s: %s", role, exc, exc_info=True)
        state["battle_games"] = list(state.get("battle_games", []))
        state["battle_result"] = {
            "skipped": True,
...
def _attach_trust_loop_artifacts(
    ...
    *,
    ...
    gate_report = build_evolution_gate_report(
        ...
        role=role,
        target_team=target_team,
        thresholds=_trust_loop_gate_thresholds(cfg or state.get("config", {}), role=role),
        scenario_replay_results=state.get("scenario_replay_report", {}).get("results") if isinstance(state.get("scenario_replay_report"), dict) else None,
    )
...
def _trust_loop_gate_thresholds(cfg: dict[str, Any], *, role: str = "") -> dict[str, Any]:
    from app.lib.evolve import EvolutionConfig

    defaults = EvolutionConfig()
    role_overrides = {}
    if role:
        rt = cfg.get("role_thresholds")
        if isinstance(rt, dict):
            role_overrides = dict(rt.get(role) or {})
    base = {
    ...
    base.update({k: v for k, v in role_overrides.items() if v is not None and k in base})
    return base
...
from ._decide import decide_node  # noqa: F401 — re-export for builder.py
