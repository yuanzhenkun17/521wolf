...
class PromptReference:
    ...
    _requested_name: str = ""
    _requested_label: str = "production"
    ...
    def compile(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if self._langfuse_prompt is not None and not self.prompt_fallback_used:
            ...
            if callable(compile_prompt):
                ...
                except Exception:  # noqa: BLE001 - prompt lookup must fail open
                    self._use_fallback()
...
def get_prompt(
    ...
    *,
    ...
    if resolved_client is None:
        return _fallback_reference(prompt_name, prompt_label, fallback)
...
def _fallback_reference(name: str, label: str, fallback: Any) -> PromptReference:
    return PromptReference(
        prompt=fallback,
        metadata=_prompt_metadata(
            name,
