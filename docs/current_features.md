...
521wolf/
├── agent/                  # Agent 层主实现
│   ├── api/                # AgentRuntime、LLMPlayerAgent、Agent 工厂
│   ├── core/               # AgentContext、记忆、信念
│   ├── decision/steps/     # 决策流水线 step
│   ├── knowledge/          # Prompt 构造、Markdown skill 加载与路由
│   ├── reasoning/          # ToT / GoT 推理内核
│   ├── learning/           # 自博弈、复盘、统计、版本评测
│   ├── learning/evolution/ # skill 自进化与版本对战
│   ├── infrastructure/     # LLM、决策日志、trace archive
│   └── common/             # 通用工具函数
├── engine/                 # 规则层和对局引擎
...
当前已经实现轻量 ToT step，用于关键动作生成多个候选方案并选择最优方案。
...
- `agent/decision/steps/reason_with_tree.py`
- `agent/reasoning/tree.py`
...
- `agent/decision/steps/record_decision.py`
...
1. ToT 当前按候选位置生成 `a/b/c`，没有严格使用模型返回的 `candidate_id`，在模型输出顺序异常时可能选错方案。
2. `521wolf.egg-info/` 是构建产物，已被 `.gitignore` 忽略，不应提交。
