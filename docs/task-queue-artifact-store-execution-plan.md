...
1. 长耗时任务在 `WOLF_USE_PG_TASK_QUEUE=true` 时写入 PostgreSQL durable queue，由单机 worker claim、lease、heartbeat、cancel、retry 和 interrupted recovery；默认 flag 关闭时保留 FastAPI `BackgroundTasks` 兼容路径。
2. benchmark、evaluation、evolution、Langfuse 验证产生的报告、manifest、diagnostics、replay bundle 等产物统一写入本地 ArtifactStore，并在 PostgreSQL 中保存 artifact metadata、sha256、size 和下载索引。
...
- 已新增本地 `LocalArtifactStore`，写入 `runs/tasks/<task_id>/<sha-prefix>/...`，并记录 `sha256`、`size_bytes`、`content_type` 和 metadata。
...
- Benchmark 工作台右侧上下文栏只在选中/活跃 run 暴露明确 `task_id` 或 `queue_task_id` 时展示队列任务和 ArtifactStore 产物，避免 legacy `run_id`/`batch_id` 误报队列任务不存在。
- Evolution 工作台右侧上下文栏只按 selected run/run summary 的明确 `task_id` 或 `queue_task_id` 展示队列任务和 ArtifactStore 产物。
...
```text
默认兼容路径：
FastAPI BackgroundTasks + wolf.ui_background_tasks + wolf.ui_task_events

queue-backed 上线路径：
wolf.ui_task_queue + TaskWorkerLoop + wolf.ui_task_events + LocalArtifactStore + wolf.ui_task_artifacts
```
...
- 前端已有独立 `/tasks` 任务中心，Benchmark/Evolution 右侧上下文栏在存在明确 `task_id`/`queue_task_id` 时展示共享 Artifact UI。
- 部署模板已补单机 worker systemd service，post-deploy smoke 会检查 `/api/tasks`、task_control health 和 artifact hash verify。

仍需保持的边界：

- flag 关闭时仍回到 `BackgroundTasks`，这是兼容路径，不是 durable queue 路径。
- 通用 `/api/tasks/{task_id}/events` 当前是 JSON replay；Benchmark/Evolution 领域 SSE 仍沿用原接口，统一 SSE/WebSocket 可以后续再收口。
- artifact retention UI、多 worker 高级调度和跨机器共享文件系统不在本轮单机上线范围内。
- full-local research runner 的本地产物 manifest 仍沿用原命名体系，暂不作为本轮 queue/artifact blocker。
...
- 新增本地文件系统 ArtifactStore：实际文件写入 `runs/tasks/<task_id>/<sha-prefix>/...`。
