...
UI_TASK_EVENT_IDENTITY_REPAIR = Path("migrations/versions/20260611_0008_repair_ui_task_event_identity.py")
...
def _ui_task_event_identity_repair_text() -> str:
    return UI_TASK_EVENT_IDENTITY_REPAIR.read_text(encoding="utf-8")
...
def test_ui_task_event_identity_repair_handles_legacy_tables() -> None:
    migration_text = _ui_task_event_identity_repair_text()
    compact_migration = re.sub(r"\s+", " ", migration_text)

    assert 'down_revision = "20260611_0007"' in migration_text
    assert "ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY" in compact_migration
    assert "pg_get_serial_sequence" in migration_text
    assert "COALESCE(MAX(id), 0) + 1" in compact_migration
    assert "setval(identity_sequence::regclass, next_id, false)" in compact_migration
