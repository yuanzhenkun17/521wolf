import {
  ...
  WebGLRenderer
    ...
    scene.background = new Color(0x010104);
    scene.fog = new FogExp2(0x05040a, 0.028);
    ...
    const MAT_BLACK = new MeshStandardMaterial({
      color: 0x060607,
      roughness: 0.78,
      metalness: 0.12
    });

    const MAT_DARK = new MeshStandardMaterial({
      color: 0x121018,
      roughness: 0.82,
      metalness: 0.05
    });

    const MAT_GOLD = new MeshStandardMaterial({
      color: 0xc79246,
      roughness: 0.34,
      metalness: 0.62
    });

    const MAT_DIM_GOLD = new MeshStandardMaterial({
      color: 0x8e6334,
      roughness: 0.5,
      metalness: 0.3
    });
    ...
    const MAT_CHAIR = new MeshStandardMaterial({
      color: 0x18151b,
      roughness: 0.86,
      metalness: 0.04
    });
    ...
    const SHARED_PLANE_GEO = new PlaneGeometry(1, 1);
    const SHARED_OUTLINE_MATERIAL = new MeshBasicMaterial({
      color: 0xdfe8ff,
      transparent: true,
      opacity: 0.2,
      depthWrite: false,
    ...
    const SHARED_FOOT_GEO = new CylinderGeometry(0.25, 0.34, 0.08, 32);
    const SHARED_FOOT_MAT = new MeshStandardMaterial({
      color: 0x21130d,
      roughness: 0.42,
      metalness: 0.22
    });
    const SHARED_RING_GEO = new RingGeometry(0.44, 0.62, 48);
    const SHARED_RING_MAT = new MeshBasicMaterial({
      color: 0xdfe8ff,
      transparent: true,
      opacity: 0.34,
      depthWrite: false,
    ...
    const SHARED_HALO_GEO = new PlaneGeometry(1.7, 2.05);
    const SHARED_HALO_MAT = new MeshBasicMaterial({
      color: 0xdfe8ff,
      transparent: true,
      opacity: 0.08,
      depthWrite: false,
    ...
    scene.add(new AmbientLight(0x4b3a58, 0.72));

    const mainLight = new PointLight(0xffd7a0, 2.85, 22, 1.42);
    mainLight.position.set(0, 5.0, 0);
    mainLight.castShadow = false;
    scene.add(mainLight);

    const frontSpot = new SpotLight(0xffc27b, 2.25, 32, Math.PI * 0.23, 0.45, 1.1);
    ...
    const leftRed = new PointLight(0xff204f, 1.45, 13, 2.0);
    leftRed.position.set(-6.6, 2.8, 1.4);
    scene.add(leftRed);

    const rightPurple = new PointLight(0x8e4dff, 1.15, 13, 2.0);
    rightPurple.position.set(6.5, 2.8, 1.4);
    scene.add(rightPurple);

    const backBlue = new PointLight(0x3764ff, 0.9, 12, 2.1);
    backBlue.position.set(0, 2.7, -6.8);
    scene.add(backBlue);

    const floor = new Mesh(
      new CircleGeometry(9.0, 160),
    ...
    const wall = new Mesh(
      new CylinderGeometry(8.55, 8.55, 5.25, 160, 1, true, Math.PI * 0.06, Math.PI * 0.88),
      new MeshStandardMaterial({
        map: starTexture,
        color: 0x110d16,
        roughness: 0.82,
    ...
    const baseRail = new Mesh(
      new CylinderGeometry(8.57, 8.57, 0.28, 160, 1, true, Math.PI * 0.06, Math.PI * 0.88),
      MAT_BLACK
    );
    ...
    createFanBarrier().then(() => markDirty());
    ...
    const centerPanel = new Mesh(
      new PlaneGeometry(2.72, 3.38),
      new MeshStandardMaterial({
        map: centerTexture,
        roughness: 0.55,
        metalness: 0.08
    ...
    async function createPosterWall() {
      ...
      POSTERS.forEach((cfg, index) => {
        const radius = 8.02;
        const theta = MathUtils.degToRad(cfg.angle);
        ...
        const frame = new Mesh(
          new BoxGeometry(panelWidth + 0.13, panelHeight + 0.16, 0.07),
          new MeshStandardMaterial({
            color: 0x130b0b,
            roughness: 0.7,
            metalness: 0.18
        ...
        const glow = new Mesh(
          new PlaneGeometry(panelWidth + 0.35, panelHeight + 0.45),
          new MeshBasicMaterial({
            color: new Color(cfg.accent),
            transparent: true,
            opacity: 0.12,
            side: DoubleSide
          })
        ...
        const lamp = new PointLight(new Color(cfg.accent), 0.58, 3.2, 1.8);
        lamp.position.set(x * 0.99, lampY, z * 0.99);
        posterGroup.add(lamp);

        const warmLamp = new PointLight(0xffc58f, 0.58, 2.7, 1.8);
        warmLamp.position.set(x * 0.988, lampY + 0.05, z * 0.988);
        posterGroup.add(warmLamp);

        const lampBody = new Mesh(
          new CylinderGeometry(0.08, 0.13, 0.28, 18),
          new MeshStandardMaterial({
            color: 0xffd9a6,
            emissive: 0xffb36a,
            emissiveIntensity: 0,
        ...
        [-0.69, 0.69].forEach((offset) => {
          ...
          const bar = new Mesh(
            new BoxGeometry(0.045, 3.72, 0.06),
            new MeshStandardMaterial({
              color: 0xd8c7a7,
              emissive: 0xd3a66b,
              emissiveIntensity: 0,
    ...
    function createOvalTable() {
      ...
      const top = new Mesh(
        new CylinderGeometry(2.42, 2.42, 0.23, 96),
        MAT_WOOD
      );
      ...
      const bevel = new Mesh(
        new TorusGeometry(2.43, 0.045, 10, 150),
        MAT_GOLD
      );
      ...
      const inner = new Mesh(
        new CylinderGeometry(1.12, 1.12, 0.075, 64),
        new MeshStandardMaterial({
          color: 0x432016,
          roughness: 0.5,
          metalness: 0.05
      ...
      const base = new Mesh(
        new CylinderGeometry(0.72, 1.03, 0.86, 42),
        MAT_WOOD
      );
    ...
    const tableSeatNumberGroup = new Group();
    ...
    function createTableSeatNumber(label, seatPosition) {
      const direction = new Vector2(seatPosition.x, seatPosition.z);
      ...
      const group = new Group();
      const tableContactY = MathUtils.lerp(1.15, 1.252, MathUtils.clamp((z + 3.2) / 5.7, 0, 1));
      group.position.set(x, tableContactY, z);
      group.rotation.y = 0;

      const shadow = new Mesh(
        new PlaneGeometry(0.24, 0.12),
        new MeshBasicMaterial({
          color: 0x120805,
          transparent: true,
          opacity: 0.3,
      ...
      plate.rotation.x = MathUtils.degToRad(-3);
    ...
    function createHumanSeatPlate(label, referencePosition) {
      ...
      const spotLight = new SpotLight(0xffe8c0, 3.0, 6, Math.PI / 5, 0.5, 1.5);
    ...
    for (let i = 0; i < 18; i++) {
      ...
      const petal = new Mesh(
        new SphereGeometry(0.075, 12, 12),
        new MeshStandardMaterial({
    ...
    const playerStandeeGroup = new Group();
        ...
        if (asOutline) {
          ...
          if (object.material) {
            ...
            if ("emissive" in object.material) {
              object.material.emissive = new Color(0x21160f);
    ...
    function createPlayerStandee(cfg, index, total) {
      ...
      outline.rotation.x = MathUtils.degToRad(-2);
      ...
      figure.rotation.x = MathUtils.degToRad(-2);
      figure.visible = false;
      figure.renderOrder = 18;
      group.add(figure);

      // Apply aspect ratio — use textureLoader.load callback (reliable js API)
      // to get image dimensions. The texture data is already cached by getCutoutTexture,
      // so this second load typically returns from browser cache instantly.
      ...
      const backPlate = new Mesh(
        new BoxGeometry(0.82, cfg.height * 0.92, 0.055),
        new MeshStandardMaterial({
          color: 0x070609,
          roughness: 0.56,
          metalness: 0.18,
      ...
      const activeHalo = new Mesh(
        SHARED_HALO_GEO,
        SHARED_HALO_MAT
      );
      activeHalo.position.set(0, 1.34, -0.08);
      activeHalo.rotation.x = MathUtils.degToRad(-2);
      ...
      if (cfg.model) {
        mountModel = () => {
          ...
          modelMountPromise = loadModel(cfg.model).then((source) => {
            ...
            group.add(model);
            const modelLight = new PointLight(0xffdfc5, 2.1, 3.0, 1.4);
      ...
      const glow = new PointLight(0xffc07a, 0.18, 2.0, 2);
    ...
    function setScenePlayers(players = [], currentSpeakerId = null) {
      ...
      const _sharedMats = new Set([SHARED_OUTLINE_MATERIAL, SHARED_FOOT_MAT, SHARED_RING_MAT, SHARED_HALO_MAT]);
      ...
      if (humanPlayer && humanVisualIndex >= 0) {
        const humanSeatPos = new Vector3(
    ...
    function scheduleModelQueue() {
      ...
      // Load models in parallel batches of 4 with short delays between batches
      const batchSize = 4;
      modelQueueTimer = window.setTimeout(() => {
        ...
        Promise.allSettled(batch.map((load) => load?.())).then(() => {
          if (queuedModelLoaders.length) scheduleModelQueue();
        });
      }, 80); // 80ms between batches instead of 900ms between individual models
    }
    ...
    function preloadQueuedModels() {
      ...
      const warmRenderModels = () => {
        // Use requestIdleCallback to defer GPU work until browser is idle,
        // falling back to setTimeout(0) if not available
    ...
    function createChair() {
      ...
      const seat = new Mesh(
        new BoxGeometry(0.92, 0.22, 0.94),
        MAT_CHAIR
      );
      ...
      const cushion = new Mesh(
        new BoxGeometry(0.78, 0.055, 0.72),
        new MeshStandardMaterial({
          color: 0x26212a,
          roughness: 0.9
        })
      ...
      const back = new Mesh(
        new BoxGeometry(0.95, 1.08, 0.19),
        MAT_CHAIR
      );
      ...
      const backCushion = new Mesh(
        new BoxGeometry(0.78, 0.82, 0.055),
        new MeshStandardMaterial({
          color: 0x25212a,
          roughness: 0.9
        })
      ...
      const armL = new Mesh(
        new BoxGeometry(0.13, 0.48, 0.72),
        MAT_CHAIR
      );
      ...
      const stem = new Mesh(
        new CylinderGeometry(0.07, 0.11, 0.62, 14),
        MAT_DARK
      );
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const leg = new Mesh(
          new BoxGeometry(0.08, 0.08, 0.62),
          MAT_DARK
        );
        leg.position.set(
          Math.sin(a) * 0.3,
    ...
    const ceilingDisc = new Mesh(
      new CircleGeometry(5.85, 160),
      new MeshStandardMaterial({
        map: starTexture,
        color: 0x090911,
        roughness: 0.64,
    ...
    [1.05, 1.82, 2.55, 3.45, 4.62, 5.14].forEach((r, idx) => {
      const ring = new Mesh(
        new TorusGeometry(r, idx > 3 ? 0.065 : 0.03, 12, 190),
    ...
    function addAstroLine(x1, z1, x2, z2, thickness = 0.018, mat = MAT_DIM_GOLD) {
      ...
      const line = new Mesh(
        new BoxGeometry(thickness, thickness, len),
        mat
      );
    ...
    const moonTexture = new CanvasTexture(moonCanvas);
    moonTexture.colorSpace = SRGBColorSpace;
    const moonCrescent = new Mesh(
    ...
    const lampRing = new Mesh(
      new TorusGeometry(0.58, 0.03, 10, 60),
      MAT_GOLD
    );
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const arm = new Mesh(
        new BoxGeometry(0.026, 0.026, 0.64),
        MAT_GOLD
      );
    ...
    function renderFrame(time = 0) {
      ...
      // Continue the loop only if we have ambient animations or haven't been idle long
      // Ambient animations need continuous rendering at ~10fps
      if (hasAmbientAnimation || idleFrameCount < IDLE_STOP_THRESHOLD) {
        animationFrameId = requestAnimationFrame(renderFrame);
      }
