...
import VoteFlowSankey from './VoteFlowSankey.vue'
...
const hasReviewFallback = computed(() =>
  ...
  || Boolean((props.game?.decisions || []).length)
)
...
function roleIconPath(role) {
  if (role.includes('预言')) return '/role-icons/预言家.png'
  if (role.includes('女巫')) return '/role-icons/女巫.png'
  if (role.includes('猎人')) return '/role-icons/猎人.png'
  if (role.includes('守卫')) return '/role-icons/守卫.png'
  if (role.includes('白狼')) return '/role-icons/白狼王.png'
  if (role.includes('狼人') || role.includes('狼王')) return '/role-icons/普通狼.png'
...
<template>
  <section class="archive-review-panel">
    ...
    <template v-if="hasReviewContent">
      ...
      <section v-if="reviewScoreCards.length" class="review-score-section">
        ...
        <div class="review-score-grid">
          <article v-for="card in reviewScoreCards" :key="'review-score-' + card.key" class="review-score-card">
            <header>
              <img class="review-role-icon" :src="roleIconPath(card.role)" alt="" />
              <span class="review-player-identity">
                <b class="review-role">{{ card.role }}</b>
                <small class="review-seat">{{ card.seat }}号</small>
              </span>
              <span class="review-overall-score">
                <small>总分</small>
                <b>{{ card.overall }}</b>
              </span>
            </header>
      ...
      <VoteFlowSankey :decisions="game.decisions || []" :players="game.players || []" />
