...
function availabilityReason(source = {}, kind = '') {
  ...
  if ((kind === 'run' || kind === 'proposal' || kind === 'gate') && (source.evolution_available === false || existing.evolution_available === false)) {
...
function buildRunEvidenceLink(source = {}, options = {}) {
  ...
  if (!runId) return disabledEvidenceLink('run', '缺少 source_run_id/run_id，无法跳转到 Evolution 运行。', options)
