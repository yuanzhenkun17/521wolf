...
const SPEECH_ACTIONS = new Set(['speak', 'sheriff_speak', 'pk_speak', 'last_word'])
const VOTE_ACTIONS = new Set(['exile_vote', 'pk_vote', 'sheriff_vote'])
...
function normalizePendingHumanAction(pending) {
  ...
  const actionType = pending.action_type || pending.type || ''
  const metadata = pending.metadata || {}
  const roleState = pending.observation?.role_state || {}
  const candidates = pending.candidates || pending.candidate_ids || []
  ...
  return {
    ...
    current_speaker_id: SPEECH_ACTIONS.has(actionType) ? pending.player_id : undefined
  }
}
...
function normalizeGameSnapshot(raw, options = {}) {
  ...
  const events = raw.logs || raw.events || []
  const game = {
    ...raw,
