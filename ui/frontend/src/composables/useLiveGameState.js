...
const hiddenMatchRecordTypes = new Set(['action_request'])
...
function isVisibleMatchRecord(log) {
  const types = primaryLogTypes(log)
  if (types.some((type) => hiddenMatchRecordTypes.has(type))) return false
  if (chatKindForLog(log) && !logText(log).trim()) return false
  return true
}
...
export function createLiveGameState(refs, helpers) {
  ...
  function chatLogEntry(log) {
    ...
    return {
      ...
      _roleIcon: player ? visibleRoleIcon(player) : '/livehall-assets/props/judge-avatar.png',
  ...
  computedState.matchRecordLogs = computed(() => {
    ...
    if (computedState.isWatch.value || refs.isReplayMode.value) {
      return logs.filter((log) => canSeeLog(log) && isVisibleMatchRecord(log)).map(matchRecordEntry)
    }
    return computedState.chatLogs.value.filter(isVisibleMatchRecord)
  })
  ...
  computedState.pageVoteTally = computed(() => {
    ...
    collectRows.forEach((row) => {
      ...
      if (sceneVotePhases.has(rowPhase) && voteActionPhase(rowPhase) !== activePhase) return
