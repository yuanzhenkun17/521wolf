...
type LeaderboardDelta = {
  target_role_role_weighted_score?: unknown
}

type LeaderboardEntry = {
  hash?: unknown
  target_role_role_weighted_score?: unknown
  target_side_win_rate?: unknown
  delta_vs_baseline?: LeaderboardDelta | null
  target_role_fallback_rate?: unknown
  recommendation?: unknown
  [key: string]: unknown
}

type NormalizedLeaderboardEntry<T extends LeaderboardEntry> = T & {
  short: string
  scorePct: number
  winRatePct: number
  deltaScore: number
...
const ROLE_META: Readonly<Record<string, RoleMeta>> = {
...
const EVOLUTION_TERMINAL_STATUSES: ReadonlySet<string> = new Set(['promoted', 'rejected', 'failed', 'completed'])
...
const RECOMMENDATION_LABELS: LabelMap = {
  promote: '建议晋升',
  reject: '建议拒绝',
  review: '待评审',
  hold: '继续观察',
...
function roleMeta(role: unknown): RoleMeta {
...
function roleLabel(role: unknown, fallback = '—'): string {
  const key = String(role || '')
  return ROLE_META[key]?.label || fallback
}

function shortId(value: unknown, length = 8, fallback = '—'): string {
...
function normalizeLeaderboardEntry<T extends LeaderboardEntry>(entry: T): NormalizedLeaderboardEntry<T> {
...
function isEvolutionBatch(batch: WorkbenchBatch | null | undefined): boolean {
  return batch?.kind === 'role_evolution_batch' || String(batch?.batch_id || '').startsWith('evo_batch_')
}

function isBenchmarkBatch(batch: WorkbenchBatch | null | undefined): boolean {
