...
function isRouteFor(
  route: Partial<RouteLike> | null | undefined,
  name: string,
): boolean {
...
export function evolutionDeepLinkPanel(
  target: Partial<EvolutionDeepLinkTarget> = {},
): string {
...
export function benchmarkBatchIdFromRoute(
  route: Partial<RouteLike> | null | undefined,
): string {
  if (!route) return "";
  const routeValue = isRouteFor(route, "benchmark")
    ? benchmarkBatchIdFromQuery(route.query || {})
    : "";
  return routeValue || benchmarkBatchIdFromHash(route.hash || "");
}

export function logsHash({
  gameId = "",
  workspace = "",
}: { gameId?: unknown; workspace?: unknown } = {}): string {
  const queryString = queryStringFromQuery(
    logsRouteQuery({ gameId, workspace }),
    ["game_id", "workspace"],
  );
  return queryString ? `#logs?${queryString}` : "#logs";
}

export function logsRouteQuery({
  gameId = "",
  workspace = "",
}: { gameId?: unknown; workspace?: unknown } = {}): LocationQueryRaw {
  const query: LocationQueryRaw = {};
  const id = textValue(gameId);
  if (id) query.game_id = id;
  const tab = optionalHistoryWorkspaceTab(workspace);
  if (tab && tab !== "phase") query.workspace = tab;
  return query;
}

export function historyDeepLinkFromQuery(
  query: QueryLike = {},
): HistoryDeepLinkTarget {
  return {
    routeHash: "#logs",
    gameId: firstTextValue(query.game_id, query.game),
    workspace: optionalHistoryWorkspaceTab(
      firstTextValue(query.workspace, query.tab),
    ),
  };
}
...
export function historyDeepLinkFromHash(value = ""): HistoryDeepLinkTarget {
  ...
  return {
    ...
    workspace: optionalHistoryWorkspaceTab(
      query.get("workspace") || query.get("tab"),
    ),
  };
}

export function historyDeepLinkFromRoute(
  route: Partial<RouteLike> | null | undefined,
): HistoryDeepLinkTarget {
  if (!route) return historyDeepLinkFromHash("");
  if (isRouteFor(route, "logs")) {
    const routeTarget = historyDeepLinkFromQuery(route.query || {});
    if (routeTarget.gameId || routeTarget.workspace || !route.hash)
      return routeTarget;
  }
  const hashTarget = historyDeepLinkFromHash(route.hash || "");
  if (hashTarget.routeHash) return hashTarget;
  return isRouteFor(route, "logs")
    ? historyDeepLinkFromQuery(route.query || {})
    : hashTarget;
}

export function evolutionDeepLinkFromQuery(
  query: QueryLike = {},
): EvolutionDeepLinkTarget | null {
  const target = {
    run_id: firstTextValue(
      query.run_id,
      query.run,
      query.source_run_id,
      query.sourceRunId,
    ),
    gate_report_id: firstTextValue(
      query.gate_report_id,
      query.gate,
      query.gateReportId,
    ),
    proposal_id: firstTextValue(
      query.proposal_id,
      query.proposal,
      query.proposalId,
    ),
    role: firstTextValue(query.role),
    version_id: firstTextValue(
      query.version_id,
      query.version,
      query.versionId,
    ),
...
export function evolutionDeepLinkFromHash(
  value = "",
): EvolutionDeepLinkTarget | null {
  const { routeHash, query } = queryFromHash(value);
...
export function evolutionDeepLinkFromRoute(
  route: Partial<RouteLike> | null | undefined,
): EvolutionDeepLinkTarget | null {
