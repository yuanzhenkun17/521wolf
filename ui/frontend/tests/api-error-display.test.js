...
test('Evolution and Logs error panels wire retry to existing refresh/reload paths', () => {
  ...
  assert.match(logsSource, /const detailRetryAvailable = computed\(\(\) => \{[\s\S]*workspaceTab\.value === 'review'[\s\S]*hasHistoryAction\('loadReview'\)[\s\S]*workspaceTab\.value === 'archive'[\s\S]*hasHistoryAction\('loadArchive'\)[\s\S]*hasHistoryAction\('loadMoreHistoryPhaseDetail'\)/)
