...
test('BenchmarkSnapshotReleasePanel exposes release readiness and disabled reasons near freeze action', () => {
  ...
  assert.match(source, /const currentSourceAudit = computed\(\(\) =>/)
...
test('BenchmarkSnapshotReleasePanel shows publish boundary, source counts, and unrankable evidence in Chinese-first copy', () => {
  const source = snapshotReleaseSource()

  assert.match(source, /当前行/)
  ...
  assert.match(source, /来源 run/)
  assert.match(source, /来源 report/)
  assert.match(source, /来源 result/)
  assert.match(source, /来源 run\/report\/result/)
  assert.match(source, /缺少来源 run ID，无法追溯评测运行/)
  assert.match(source, /缺少来源 report ID，无法追溯评测报告/)
  assert.match(source, /缺少来源 result ID，无法追溯结果批次/)
  assert.match(source, /存在评测边界告警，需先处理后再冻结/)
  assert.match(source, /不可排名证据/)
  assert.match(source, /随快照保留/)
  assert.match(source, /不会进入正式排名/)
  assert.match(source, /Seed Set/)
  assert.match(source, /Config Hash/)
...
test('BenchmarkSnapshotReleasePanel makes release identity, source lineage, and boundary warnings hard blockers', () => {
  const source = snapshotReleaseSource()
  ...
  assert.match(source, /key: 'source-run'[\s\S]*required: true[\s\S]*blockedReason: '缺少来源 run ID/)
  assert.match(source, /key: 'source-report'[\s\S]*required: true[\s\S]*blockedReason: '缺少来源 report ID/)
  assert.match(source, /key: 'source-result'[\s\S]*required: true[\s\S]*blockedReason: '缺少来源 result ID/)
...
test('BenchmarkSnapshotReleasePanel keeps warm snapshot release styling', () => {
  ...
  assert.match(source, /\.snapshot-gate-check\.blocked/)
})
...
test('BenchmarkSnapshotReleasePanel SFC compiles after release gate wiring', () => {
  ...
  assert.match(script.content, /currentRowSourceAudit/)
