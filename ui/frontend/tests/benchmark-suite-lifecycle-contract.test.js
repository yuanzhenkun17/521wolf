...
test('BenchmarkSuiteRail renders selected suite spec, lifecycle, seed, gates, metrics, and judge details', () => {
  ...
  assert.match(source, /label: '版本'/)
  assert.match(source, /label: '层级'/)
  assert.match(source, /label: '对象类型'/)
  assert.match(source, /label: '使用边界'/)
  assert.match(source, /label: '非重叠组'/)
  assert.match(source, /label: '不可变'/)
  assert.match(source, /重叠警告/)
  assert.match(source, /selectedSeedWarnings/)
  assert.match(source, /seedWarningCount/)
