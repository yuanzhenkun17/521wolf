...
test('Evolution workbench shell exposes the persistent right context rail', () => {
  ...
  assert.doesNotMatch(shell, /高风险动作/)
  assert.doesNotMatch(shell, /const riskActionRows = computed\(\(\) => \[/)
...
test('EvolutionPage wires desktop context props without changing the seven work areas', () => {
  ...
  assert.match(page, /const selectedCanReview = computed\(\(\) => evolutionStore\.selectedCanReject\)/)
  assert.match(page, /const selectedCanTerminate = computed\(\(\) => evolutionStore\.selectedCanTerminate\)/)
  assert.match(page, /:run-rows="evolutionStore\.runRows"/)
  assert.match(page, /:selected-run="evolutionStore\.selectedRun"/)
  assert.match(page, /:selected-proposal-review="evolutionStore\.selectedProposalReview"/)
  assert.match(page, /:selected-can-promote="selectedCanPromote"/)
  assert.match(page, /:selected-reject-disabled-reason="evolutionStore\.selectedRejectDisabledReason"/)
  assert.match(page, /:selected-terminate-disabled-reason="evolutionStore\.selectedTerminateDisabledReason"/)
  assert.match(page, /:selected-rollback-disabled-reason="evolutionStore\.selectedRollbackDisabledReason"/)
  assert.match(page, /grid-template-columns:\s*252px minmax\(0, 1fr\) 300px/)
