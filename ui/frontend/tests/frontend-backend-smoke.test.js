...
test('MatchPage mobile safe-area source contract keeps errors on ApiErrorPanel', () => {
  ...
  assert.match(matchPage, /inlineMatchNotice = computed\(\(\) => inlineNoticeForDisplay\(matchNotice\.value\)\)/)
  assert.match(matchPage, /matchErrorNotice = computed\(\(\) => matchPanelErrorForNotice\(matchNotice\.value\)\)/)
