...
test('history review and archive workspace tabs expose lazy load state badges', () => {
  ...
  assert.match(logsPageSource, /const reviewTabState = computed\(\(\) => asyncTabState\(\{[\s\S]*loading: props\.reviewLoading[\s\S]*loaded: reviewLoaded\.value[\s\S]*error: Boolean\(selectedReview\.value\?\.error\)/)
  assert.match(logsPageSource, /const archiveTabState = computed\(\(\) => asyncTabState\(\{[\s\S]*loading: props\.archiveLoading[\s\S]*loaded: archiveLoaded\.value[\s\S]*error: Boolean\(selectedArchive\.value\?\.error\)/)
