...
test('BenchmarkPage is wired to the shared Lab shell without changing benchmark business panels', () => {
  ...
  assert.match(benchmark, /const benchmarkCommandMetaRows = computed\(\(\) => \[[\s\S]*key: 'mode'[\s\S]*key: 'subject'[\s\S]*key: 'games'[\s\S]*key: 'units'[\s\S]*key: 'budget'[\s\S]*key: 'launch'/)
  assert.match(benchmark, /<LabWorkbenchShell[\s\S]*v-model:active-tab="activeView"[\s\S]*workbench-key="benchmark"[\s\S]*:tabs="navTabs"[\s\S]*:meta="benchmarkCommandMetaRows"[\s\S]*action-label="刷新"[\s\S]*@action="refresh"/)
  ...
  assert.match(benchmark, /<template #context>[\s\S]*class="bench-context-panel"[\s\S]*当前套件[\s\S]*评测边界[\s\S]*入榜门禁[\s\S]*运行上下文[\s\S]*诊断概览[\s\S]*发布材料[\s\S]*<\/template>/)
  ...
  assert.doesNotMatch(benchmark, /const labHeaderMeta|:meta="labHeaderMeta"|<template #boundary>|<BenchmarkBoundaryBar|:show-header="false"|<template #tabs-actions>|bench-tabs-refresh/)
})
...
test('Benchmark launch and target panels keep Chinese-first benchmark terminology', () => {
  ...
  assert.doesNotMatch(target, /模型评测写入 scope=model|角色版本评测写入 scope=role_version|model\/runtime config|启动 payload|roles 或 target_versions/)

  assert.match(benchmark, /const currentScopeLabel = computed/)
  assert.match(benchmark, /label: '比较边界', value: currentScopeLabel\.value/)
  assert.match(benchmark, /label: '评测集'/)
  assert.match(benchmark, /label: '种子集'/)
  assert.match(benchmark, /quick:\s*'quick 快速'/)
  ...
  assert.match(benchmark, /label: '调用单位'/)
  assert.match(benchmark, /label: '启动门禁'/)
  assert.match(benchmark, /超预算原因/)
  assert.match(benchmark, /仅预估不启动|只做预算预检/)
  assert.doesNotMatch(benchmark, /label: 'Scope'|label: 'Evaluation Set'|label: 'Seed Set'|label: 'Dry Run'|label: '预计 Token'|label: '预计 token'|label: '预检模式'|label: '并发策略'|预算与停止线提示|<dt>Evaluation Set<\/dt>|<dt>Seed Set<\/dt>|个 Judge 并发进程|个 Judge 并发任务|预计单位/)
...
test('EvolutionPage uses EvolutionWorkbenchShell directly without LabWorkbenchShell bridge', () => {
  const evolution = readSource('../src/pages/EvolutionPage.vue')

  assert.doesNotMatch(evolution, /import LabWorkbenchShell/)
  assert.match(evolution, /<EvolutionWorkbenchShell[\s\S]*v-model:active-tab="activeTab"[\s\S]*@select-role="evolutionStore\.selectRole"/)
...
test('Lab shell and EvolutionPage keep the warm logbook palette bridge', () => {
  ...
  assert.doesNotMatch(evolution, /--logbook-bg:\s*var\(--workbench-logbook-bg/)
  ...
  assert.match(leaderboard, /\.evo-leaderboard-score em\s*\{/)
  assert.match(leaderboard, /is-baseline/)
  assert.match(versions, /PATTERN_STATUS_COLORS[\s\S]*active:\s*'var\(--evo-accent\)'/)
  assert.doesNotMatch(reject, /--logbook-bg-texture:[\s\S]*repeating-linear-gradient/)
  ...
  assert.doesNotMatch(trust, /--logbook-bg-texture:[\s\S]*repeating-linear-gradient/)
