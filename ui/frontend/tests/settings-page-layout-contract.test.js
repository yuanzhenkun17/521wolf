...
test('SettingsPage follows local secret safety rules', () => {
  ...
  assert.match(source, /要取消降级：设置 LANGFUSE_CAPTURE_INPUT_OUTPUT=true、LANGFUSE_SAMPLE_RATE=1、LANGFUSE_ENVIRONMENT 和 LANGFUSE_RELEASE 后重启。/)
  ...
  assert.match(source, /function localizeStorageMessage\(message: string\): string/)
  assert.match(source, /PostgreSQL 设置存储已就绪/)
  assert.match(source, /\.settings-storage-list span \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto;/)
  assert.match(source, /\.settings-context-kpis \{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/)
  assert.match(source, /\.settings-context-kpis b \{[\s\S]*overflow-wrap: anywhere;/)
  ...
  assert.match(source, /gate_blocked\./)
  assert.match(source, /门禁未通过，请先处理下方阻断项/)
  assert.match(source, /部署入口健康检查未就绪，需要先恢复 API 健康状态/)
  assert.match(source, /benchmark_start: '启动评测'/)
  assert.match(source, /evolution_start: '启动自进化'/)
  ...
  assert.match(source, /\.settings-alert-list span \{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/)
  assert.match(source, /class="settings-profile-id"/)
  assert.match(source, /\.settings-context-kpis \.settings-profile-id \{/)
  assert.doesNotMatch(source, /class="settings-context-run-id"/)
  assert.doesNotMatch(source, /class="settings-profile-note"/)
...
test('SettingsPage explains profile recovery state and prevents testing stale form drafts', () => {
  ...
  assert.match(source, /const activeScopedEnvLocked = computed/)
  assert.match(source, /const envLockSourceNames = computed/)
  assert.match(source, /const scopedDefaultTitle = computed/)
  assert.match(source, /function profileCanLaunch\(profile: ModelProfile\): boolean[\s\S]*profile\.enabled && profile\.has_api_key/)
  assert.match(source, /activeScopedEnvLocked\.value \? '环境变量默认模型'/)
  assert.match(source, /WEREWOLF_LLM_\* 环境变量/)
  assert.match(source, /class="settings-profile-row settings-env-profile-row"/)
  assert.match(source, /v-if="!scopedProfiles\.length && !activeScopedEnvLocked"/)
