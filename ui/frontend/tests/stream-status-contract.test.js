...
test('TopNav derives StreamStatusBadge from existing activeSession stream fields', () => {
  ...
  assert.match(source, /const propActiveSession = computed<StreamAwareActiveSession>\(\(\) => hasExplicitTopNavProp\('activeSession'\) \? props\.activeSession \|\| \{\} : \{\}\)/)
  assert.match(source, /const session = effectiveActiveSession\.value/)
  assert.match(source, /return propActiveSession\.value/)
