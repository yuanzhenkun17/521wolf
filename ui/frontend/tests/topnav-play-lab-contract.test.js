...
test('TopNav buttons show current page state without changing App routing events', () => {
  ...
  assert.match(source, /const configuredTtsAvailable = computed\(\(\) => hasExplicitUiProp\('ttsAvailable'\) \? props\.ttsAvailable : uiStore\.ttsAvailable\)/)
  assert.match(source, /const effectiveTtsAvailable = computed\(\(\) => !replayStore\.isReplayMode && Boolean\(configuredTtsAvailable\.value\)\)/)
  assert.match(source, /const ttsDisabledTitle = computed\(\(\) => replayStore\.isReplayMode \? '回放不支持发言朗读' : '发言朗读未配置'\)/)
  ...
  assert.match(appSource, /const audioError = computed\(\(\) => String\(audio\.ttsError\?\.value \|\| ''\)\)/)
  assert.match(appSource, /const toastError = computed\(\(\) => uiStore\.errorMessage \|\| audioError\.value\)/)
