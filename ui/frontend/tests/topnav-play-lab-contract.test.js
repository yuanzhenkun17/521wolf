...
test('TopNav buttons show current page state without changing App routing events', () => {
  ...
  assert.match(source, /return props\.activeView \|\| routeActiveView\.value \|\| storeActiveView\.value \|\| 'lobby'/)
  ...
  assert.match(appSource, /const audioEnabled = computed\(\(\) => uiStore\.audioEnabled\)/)
  assert.match(appSource, /const ttsEnabled = computed\(\(\) => uiStore\.ttsEnabled\)/)
  assert.match(appSource, /const ttsAvailable = computed\(\(\) => uiStore\.ttsAvailable\)/)
  ...
  assert.match(appSource, /const activeSession = computed\(\(\) => sessionStore\.activeSession\)/)
  assert.match(appSource, /const isNight = computed\(\(\) => gameStore\.isNight\)/)
  assert.match(appSource, /const topNavActiveView = computed\(\(\) => replayStore\.isReplayMode \? 'logs' : activeAppView\.value\)/)
  assert.match(appSource, /const toastError = computed\(\(\) => uiStore\.errorMessage\)/)
  assert.match(appSource, /return isReturnableGame\(gameStore\.liveGame\)/)
