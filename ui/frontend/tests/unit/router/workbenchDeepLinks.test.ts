...
test("extracts benchmark batch ids from route query aliases before legacy hash fallback", () => {
  ...
  assert.equal(
    benchmarkBatchIdFromQuery({ source_run_id: "bench-run-a" }),
    "bench-run-a",
  );
  assert.equal(
    benchmarkBatchIdFromQuery({ run_id: ["run-a", "run-b"] }),
    "run-a",
  );
  assert.equal(
    benchmarkBatchIdFromHash("#benchmark?source_run_id=bench-from-hash"),
    "bench-from-hash",
  );
...
test("builds logs hashes with optional game and non-default workspace query", () => {
  ...
  assert.equal(
    logsHash({ gameId: "game/1", workspace: "phase" }),
    "#logs?game_id=game%2F1",
  );
  assert.equal(
    logsHash({ gameId: "game/1", workspace: "archive" }),
    "#logs?game_id=game%2F1&workspace=archive",
  );
  assert.equal(logsHash({ workspace: "review" }), "#logs?workspace=review");
});

test("builds logs route query for router-owned navigation", () => {
  assert.deepEqual(logsRouteQuery(), {});
  assert.deepEqual(logsRouteQuery({ gameId: "game/1", workspace: "phase" }), {
    game_id: "game/1",
  });
  assert.deepEqual(logsRouteQuery({ gameId: "game/1", workspace: "archive" }), {
    game_id: "game/1",
    workspace: "archive",
  });
  assert.deepEqual(logsRouteQuery({ workspace: "unknown" }), {});
});

test("extracts logs deep link targets from query and legacy hash aliases", () => {
  assert.deepEqual(
    historyDeepLinkFromQuery({ game_id: "game-a", workspace: "archive" }),
    {
      routeHash: "#logs",
      gameId: "game-a",
      workspace: "archive",
  ...
  assert.deepEqual(
    historyDeepLinkFromHash("#logs?game_id=game-c&workspace=unknown"),
    {
      routeHash: "#logs",
      gameId: "game-c",
      workspace: "",
